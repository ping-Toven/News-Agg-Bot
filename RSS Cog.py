...
conn = sqlite3.connect(RSS_database)
...
if conn is not None:
    db.create_table(db, conn, sql_create_feeds_table)
...
class RSS_Cog(commands.Cog):
    ...
    async def add_rss(self, ctx, feed_url: Option(str, "Enter the URL of the RSS Feed you'd like to add")):
        ...
        status = db.add_feed(db, conn, feed_url, guild_id)
    ...
    async def remove_rss(self, ctx, feed_url: str):
        ...
        channel_id = str(db.get_posting_channel(db, conn, guild_id))
        channel_mention = "<#" + channel_id + ">"
        status = db.remove_feed(db, conn, feed_url, guild_id)
        await ctx.respond(status + f" in {channel_mention}")
